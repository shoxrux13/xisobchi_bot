...
export class IncomeScene {
  ...
  @SceneEnter()
  async onEnter(@Ctx() ctx: Scenes.SceneContext & { scene: { state: IncomeState } }): Promise<void> {
    ...
    await ctx.reply(
      this.t(settings.languageCode, 'income.ask_amount'),
      {
        parse_mode: 'HTML',
  ...
  @On('text')
  async onText(@Ctx() ctx: Scenes.SceneContext & { scene: { state: IncomeState }; message: { text: string } }): Promise<void> {
    ...
    if (state.step === 'amount') {
      ...
      if (!amount) {
        await ctx.reply(this.t(lang, 'income.invalid_amount'), { parse_mode: 'HTML' });
        return;
      }
      state.amount = amount;
      state.step = 'category';
      ...
      await ctx.reply(
        this.t(lang, 'income.ask_category'),
        {
          parse_mode: 'HTML',
  ...
  @On('callback_query')
  async onCallback(@Ctx() ctx: Scenes.SceneContext & { scene: { state: IncomeState }; callbackQuery: { data?: string } }): Promise<void> {
    const state = ctx.scene.state;
    const lang = state.lang ?? 'uz';
  ...
  private async saveTransaction(
    ctx: Scenes.SceneContext & { scene: { state: IncomeState } },
    state: IncomeState,
    note: string | null,
  ): Promise<void> {
    const lang = state.lang ?? 'uz';
  ...
  @Command('cancel')
  async onCancel(@Ctx() ctx: Scenes.SceneContext & { scene: { state: IncomeState } }): Promise<void> {
